...
with col1:
    ...
    with tab1:
        st.caption("Кликните на белое поле ниже")
            ...
            if not df_canvas.empty:
                for _, row in df_canvas.iterrows():
                    if not ((st.session_state.all_points['x'] == row['left']) & 
                        ...
                        new_pt = pd.DataFrame({'x': [row['left']], 'y': [row['top']], 'weight': [10.0]})
    ...
    with tab2:
        with st.form("coord_form", clear_on_submit=True):
            ...
            nw = c3.number_input("Масса (W)", 0.1, 1000.0, 50.0)
    ...
    st.caption("Редактируйте массу прямо в таблице. Удалить строку: выделите и нажмите Del.")
...
with col2:
    ...
    if not df.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                color='#0055ff',        # Насыщенный синий
                size=df['weight'].astype(float)*0.1 + 12, # Зависимость размера от массы
                opacity=1.0, 
                line=dict(width=2, color='white') # Белый контур для яркости
            ),
            name="Точки массы"
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                ...
                size=20, 
                symbol='x-thin-open', 
                line=dict(width=4, color='#ff0000') # Очень жирный прицел
            ),
            name="Центр масс"
        ))
        ...
        m3.metric("Масса системы", f"{total_w:.1f}")
    else:
        st.info("Добавьте данные, чтобы увидеть график.")
