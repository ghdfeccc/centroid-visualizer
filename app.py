...
st.set_page_config(page_title="Центр Масс | Стабильная версия", layout="wide")

# 2. Дизайн
st.markdown("""
    <style>
    .stApp { background-color: #f0f2f6; background-image: linear-gradient(#d1d4f9 1px, transparent 1px), linear-gradient(90deg, #d1d4f9 1px, transparent 1px); background-size: 30px 30px; }
    [data-testid="stVerticalBlock"] > div:has(div.stMetric), .stTabs, .stDataEditor, [data-testid="stForm"] { background-color: rgba(255, 255, 255, 0.98); padding: 25px; border-radius: 15px; box-shadow: 0 8px 20px rgba(0,0,0,0.1); border: 1px solid #c1c4f7; }
...
with col1:
    ...
    with tab1:
        st.caption("Поставьте точки и нажмите кнопку ниже, чтобы зафиксировать их")
        canvas_result = st_canvas(
            ...
            drawing_mode="point",
            update_streamlit=False, # ОТКЛЮЧАЕМ авто-обновление, чтобы избежать петли
            point_display_radius=6,
            key=f"canvas_{st.session_state.sync_key}", 
        )
        
        # Кнопка для переноса точек с холста в таблицу
        if st.button("✅ Добавить точки с холста"):
            if canvas_result.json_data is not None:
                df_canvas = pd.json_normalize(canvas_result.json_data["objects"])
                if not df_canvas.empty:
                    new_pts = df_canvas[['left', 'top']].copy()
                    new_pts.columns = ['x', 'y']
                    new_pts['weight'] = 1.0
                    # Объединяем и удаляем дубликаты
                    st.session_state.all_points = pd.concat([st.session_state.all_points, new_pts]).drop_duplicates(subset=['x', 'y']).reset_index(drop=True)
                    st.rerun()
    ...
    edited_df = st.data_editor(st.session_state.all_points, num_rows="dynamic", use_container_width=True, key=f"editor_{st.session_state.sync_key}")
    
    # Синхронизация изменений из таблицы в сессию
    if not edited_df.equals(st.session_state.all_points):
        st.session_state.all_points = edited_df
        # Если удалили строку, нужно сбросить холст
...
with col2:
    ...
    if not df.empty:
        ...
        fig.add_trace(go.Scatter(x=[cx], y=[cy], mode='markers', marker=dict(color='#ff0000', size=18, symbol='circle', line=dict(width=3, color='white')), name="Центр"))
