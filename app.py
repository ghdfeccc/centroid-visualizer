...
st.set_page_config(page_title="Центр Масс | Mobile Fix", layout="wide")

# 2. Улучшенный CSS специально для ТЕЛЕФОНОВ
st.markdown("""
    <style>
    ...
    /* Блокировка прокрутки браузером при касании холста (Критично для мобильных!) */
    canvas, .stCanvas, iframe {
    ...
    h1 { color: #003399; background: white; padding: 10px; border-radius: 12px; border-left: 8px solid #ff0000; }
    </style>
...
with col1:
    ...
    with tab1:
        canvas_result = st_canvas(
            ...
            point_display_radius=8,   # Сделали точку еще крупнее для пальца
            key="canvas_mobile_fixed",
        )
    ...
    st.session_state.all_points = st.data_editor(
        st.session_state.all_points, num_rows="dynamic", use_container_width=True
    )
...
with col2:
    ...
    if not df.empty:
        ...
        fig.add_trace(go.Scatter(x=[cx], y=[cy], mode='markers',
                                 ...
                                 line=dict(width=3, color='white')), name="Центр"))
        ...
        st.metric("Центр масс", f"{cx:.1f}, {cy:.1f}")
    else:
